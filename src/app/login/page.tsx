...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <motion.div
          animate={{ scale: [1, 1.2, 1], opacity: isDark ? [0.1, 0.2, 0.1] : [0.05, 0.1, 0.05] }}
          transition={{ duration: 8, repeat: Infinity }}
          className={`absolute top-1/4 right-1/4 w-96 h-96 rounded-full blur-3xl ${isDark ? 'bg-blue-500/20' : 'bg-blue-300/30'}`}
        />
        <motion.div
          animate={{ scale: [1, 1.3, 1], opacity: isDark ? [0.1, 0.15, 0.1] : [0.05, 0.08, 0.05] }}
          transition={{ duration: 10, repeat: Infinity, delay: 2 }}
          className={`absolute bottom-1/4 left-1/4 w-96 h-96 rounded-full blur-3xl ${isDark ? 'bg-emerald-500/20' : 'bg-emerald-300/30'}`}
        />
      </motion.div>
      ...
      <div className="absolute top-6 left-6 z-20">
        ...
        >
          ...
          <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Accounting</span>
        </motion.div>
      </div>
        ...
        <Card className={`backdrop-blur-xl shadow-2xl ${isDark ? 'border-slate-700/50 bg-slate-900/80' : 'border-slate-200 bg-white/80'}`}>
          <CardHeader className="text-center space-y-2 pb-6">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4, type: 'spring' }}>
              <CardTitle className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Welcome Back</CardTitle>
            </motion.div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
              <CardDescription className={isDark ? 'text-slate-400' : 'text-slate-500'}>Sign in to your accounting dashboard</CardDescription>
            </motion.div>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <motion.div initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="space-y-2">
                ...
                <Input
                  ...
                  required
                  className={`${isDark ? 'bg-slate-800/50 border-slate-700 text-white placeholder:text-slate-500' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400'} h-11`}
                />
              </motion.div>
