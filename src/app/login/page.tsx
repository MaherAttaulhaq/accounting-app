    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <motion.div
          ...
          className={`absolute top-1/4 -right-20 sm:right-1/4 w-48 sm:w-96 h-48 sm:h-96 rounded-full blur-3xl ${isDark ? 'bg-blue-500/20' : 'bg-blue-300/30'}`}
        />
        <motion.div
          animate={{ scale: [1, 1.3, 1], opacity: isDark ? [0.1, 0.15, 0.1] : [0.05, 0.08, 0.05] }}
          transition={{ duration: 10, repeat: Infinity, delay: 2 }}
          className={`absolute -bottom-20 sm:bottom-1/4 -left-20 sm:left-1/4 w-48 sm:w-96 h-48 sm:h-96 rounded-full blur-3xl ${isDark ? 'bg-emerald-500/20' : 'bg-emerald-300/30'}`}
        />
      </motion.div>
        ...
        <Card className={`backdrop-blur-xl shadow-2xl ${isDark ? 'border-slate-700/50 bg-slate-900/80' : 'border-slate-200 bg-white/80'}`}>
          <CardHeader className="text-center space-y-2 pb-4 sm:pb-6">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4, type: 'spring' }}>
              <CardTitle className={`text-2xl sm:text-3xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Welcome Back</CardTitle>
            </motion.div>
