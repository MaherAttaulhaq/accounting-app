...
export function Header({ user }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-md border-b border-slate-200/50 z-50">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="md:hidden absolute top-16 left-0 right-0 bg-white border-b shadow-lg p-4"
          >
            {navItems.map((item) => {
              const isActive = pathname === item.href;
              return (
